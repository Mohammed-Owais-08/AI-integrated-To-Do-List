...
def suggest_order():
    todos = Todo.query.filter_by(done=False).all()
    prompt = "You are a productivity assistant. Please sort the tasks in order of priority, with the most important and impactful tasks first and the least important ones last. Also, give me the best order to complete them today.. Return ONLY a JSON array of objects like [{\"id\":1,\"title\":\"Task title\",\"reason\":\"short explanation\"}].\n\nTasks:\n"
    for t in todos:
